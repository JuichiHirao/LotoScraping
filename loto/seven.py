...
url = 'https://www.mizuhobank.co.jp/takarakuji/loto/backnumber/lt7-201612.html'
...
for i, table in enumerate(type_tk):
    ...
    lottery_num = []
    extension_list = table.find_all('td', class_="extension alnCenter")
    for idx, ext in enumerate(extension_list):
        lottery_num.append(ext.text)

    ext_green_list = table.find_all('td', class_="extension alnCenter green")
    ...
    cursor = db.cursor()
    carry_over = int(lottery_info[6][0])
    # print (str(carry_over))
    cursor.execute(sql,
                   [datetime.strptime(lotteries_date, '%Y-%m-%d'), times, ",".join(lottery_num),
                    lottery_info[0][0], lottery_info[0][1], lottery_info[1][0], lottery_info[1][1],
                    lottery_info[2][0], lottery_info[2][1], lottery_info[3][0], lottery_info[3][1],
                    lottery_info[4][0], lottery_info[4][1],
                    lottery_info[5][0], lottery_info[5][1], lottery_info[6][0], lottery_info[6][1],
    ...
    with db.xact():
        ...
        if cnt > 0:
            ...
            print("DATABASE NOT EXIST " + times + " [" + str(row["created_at"]) + "]")
    '''
